<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:84:"F:\wamp\www\htt\public/../application/admin2\view\..\..\common\view\table\index.html";i:1520391765;}*/ ?>
<!doctype html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>好太太上市活动后台管理系统</title>
...
<div class="x-body">
    ...
    <div class="layui-row">
        <?php if($search): ?>
            <form method="get" action="<?php echo url($route); ?>" class="layui-form layui-col-md12 x-so">
                <?php if(is_array($search) || $search instanceof \think\Collection): $i = 0; $__LIST__ = $search;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
    ...
    <xblock>
        ...
        <?php endif; if(is_array($tool) || $tool instanceof \think\Collection): $i = 0; $__LIST__ = $tool;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
            <?php echo $v; endforeach; endif; else: echo "" ;endif; ?>
    ...
    <table class="layui-table">
        ...
        <tr>
            ...
            <?php if(is_array($thead) || $thead instanceof \think\Collection): $i = 0; $__LIST__ = $thead;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
...
<script type="text/javascript">
    ...
    function get_ajax(url){
        var token=$('[name="__token__"]').val();
        layer.confirm('只会发送给未填写地址的用户，确定发送吗？',function(index){
            //捉到所有被选中的，发异步进行删除
            //捉到所有被选中的，发异步进行删除
